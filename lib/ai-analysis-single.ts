...
第1步：判断作品类型
先判断这是哪类设计作品（海报/UI界面/插画/包装/品牌/展板/模型/其他），确定适用的评估视角。

第2步：红牌规则检查
逐条检查以下 7 条红牌规则。每条被触发的规则，将其描述写入 redFlags 数组：
...
必须基于你看到的画面内容，为四位不同视角的导师各写一段点评。每位导师点评必须包含：
1. 至少 1 个从画面中直接观察到的具体证据（不许凭空编造）
2. 基于该证据给出该角色视角的专业判断

画面证据类型参考（必须选择实际看到的来写）：
- 版式：主次顺序、图文比例、网格对齐、视觉重心在哪
- 色彩：配色策略、色彩冲突或和谐程度、色调情绪
- 字体：字体选择、字号层级、标题与正文的关系
- 图像：图片质量、模型渲染、摄影构图、插图风格
- 元素：某具体元素的设计处理、图标风格、装饰与功能的关系
- 可读性：某文字区域是否可读、信息层级是否清楚
- 展板特有：图纸密度、爆炸图/剖面图/效果图分布、标题信息完整性
- UI 特有：界面状态覆盖、组件一致性、流程反馈、信息架构
- 海报特有：主视觉冲击力、文字信息可读距离、传播场景适配
- 包装/品牌：品牌调性统一性、图形系统完整性、材质/结构表达
- 概念作品：思辨性表达、研究过程可视化、命题的清晰度

四位导师各自的关注重点（必须严格区分，不能写内容相似的点评）：

🎓 毕业导师 — 关注课题完整度、概念推导、学术说服力、过程证据：
- 从画面中能看到多少设计推导过程？概念是否讲得清楚？
- 导师会在答辩中追问什么问题？哪些地方缺少过程材料？
- 学术表达是否规范？结论是否有支撑？

💼 设计总监 — 关注审美判断、视觉成熟度、商业落地、设计语言一致性：
- 第一眼的审美判断是什么？整体视觉是否"成立"？
- 主视觉焦点在哪？它有效吗？有没有干扰元素？
- 如果这是一个商业方案，客户会质疑什么？

🤝 企业面试官 — 关注作品集表达、项目过程、岗位匹配、面试可讲性：
- 这张图如果放进作品集，面试官会问什么问题？
- 作品展示了作者什么能力？缺少什么能力证据？
- 项目过程是否可追溯？决策是否有依据？

🔍 用户研究员 — 关注用户场景、目标人群、信息理解、使用/传播逻辑：
- 谁会看到这张图？在什么情境下看？他们能理解吗？
- 信息传达是否有效？用户会从哪里开始看？
- 传播或使用中会遇到什么认知障碍？
...
请严格按照评分流程（类型判断→红牌检查→维度评分→等级判定→calibrationNote→mentorReviews），逐维度分析并生成四位导师点评。注意：mentorReviews 中每位导师必须引用画面具体证据，不得写泛泛而谈的模板话。只返回 JSON，不要其他文字。`
